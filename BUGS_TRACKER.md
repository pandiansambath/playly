...
- [x] **B2. Video → audio toggle silently mutes audio** — Fixed. Root cause was a race in `ensureAnalyser`: when the `AudioContext` was suspended (browser policy after inactivity), the `.then()` after `_audioCtx.resume()` read `showVideo` from the store — but React hadn't flushed the toggle yet, so the OLD value (`true`) was used → callback set `_boost.gain = 0`, re-muting the audio that the synchronous code had just unmuted. Fix: the resume callback no longer touches gain/muted; instead `handleVideoToggle` is now async and `await`s a new `resumeAudioGraph()` before setting gain/muted in the OFF path. Synchronous, deterministic.
